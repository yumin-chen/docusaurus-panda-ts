            ...
            switch (diffs[pointer][0]) {
                ...
                case DiffOp.Equal:
                    ...
                    if (count_delete + count_insert > 1) {
                        ...
                        if (count_delete === 0) {
                            diffs.splice(pointer - count_insert, count_delete + count_insert, [DiffOp.Insert, text_insert]);
                        }
                        else if (count_insert === 0) {
                            diffs.splice(pointer - count_delete, count_delete + count_insert, [DiffOp.Delete, text_delete]);
                        }
                        else {
                            diffs.splice(pointer - count_delete - count_insert, count_delete + count_insert, [DiffOp.Delete, text_delete], [DiffOp.Insert, text_insert]);
                        }
                        pointer =
                            pointer -
                                count_delete -
        ...
        for (let x = 0; x < patches.length; x++) {
            ...
            while (bigpatch.diffs.length !== 0) {
                ...
                while (bigpatch.diffs.length !== 0 &&
                    ...
                    else if (diff_type === DiffOp.Delete &&
...
export { DiffMatchPatch, DiffOp, PatchOperation };
//# sourceMappingURL=diff-match-patch-ts.mjs.map
