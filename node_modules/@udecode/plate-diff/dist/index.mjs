...
import { isText as isText4 } from "@udecode/plate-common/server";
import isEqual4 from "lodash/isEqual.js";
...
var childrenOnlyStrategy = (node, nextNode, options) => {
  if (node.children != null && nextNode.children != null && isEqual(
...
import { Path as Path2, createEditor, withoutNormalizing } from "slate";
...
var InlineNodeCharMap = class {
  ...
  replaceCharWithNode(haystack, needle, replacementNode) {
    return haystack.flatMap((haystackNode) => {
      if (!isText(haystackNode))
        return [haystackNode];
      const splitText = haystackNode.text.split(needle);
      if (splitText.length === 1)
        return [haystackNode];
      const replacementWithProps = __spreadValues(__spreadValues({}, replacementNode), getNodeProps(haystackNode));
...
import { addRangeMarks } from "@udecode/plate-common/server";
...
var applyMergeNode = (editor, apply, op) => {
  const oldNode = Node.get(editor, op.path);
...
var commitChangesToDiffs = (editor, { getDeleteProps, getInsertProps, getUpdateProps }) => {
  withoutRecordingOperations(editor, () => {
    ...
    flatUpdates.forEach(({ newProperties, properties, range }) => {
      const node = Node.get(editor, range.anchor.path);
      addRangeMarks(
        editor,
    ...
    editor.insertedTexts.forEach(({ node, rangeRef }) => {
      ...
      if (range) {
        addRangeMarks(editor, getInsertProps(node), { at: range });
      }
      rangeRef.unref();
    });
  });
...
var flattenPropsChanges = (editor) => {
  ...
  const flatUpdates = flatRanges.map((flatRange) => {
    ...
    if (isEqual2(initialProps, finalProps))
      return null;
    const properties = {};
    const newProperties = {};
...
function transformDiffTexts(nodes, nextNodes, options) {
  ...
  const charGenerator = unusedCharGenerator({
    ...
    skipChars: nodes.concat(nextNodes).filter(isText2).map((n) => n.text).join("")
  });
...
function splitTextNodes(node, split, options) {
  ...
  for (let i = 0; i < split.length - 1; i++) {
    ...
    splitPath = Path2.next(splitPath);
...
function isEqualNode(value, other) {
  return isElement(value) && isElement(other) && value.children !== null && other.children !== null && isEqual3(copyWithout(value, ["children"]), copyWithout(other, ["children"]));
}
function isEqualNodeChildren(value, other) {
  if (isElement(value) && isElement(other) && isEqual3(value.children, other.children)) {
    return true;
  }
  return isText3(value) && isText3(other) && isEqual3(value.text, other.text);
}
...
function transformDiffDescendants(diff, _a) {
  ...
  const areNodeListsEquivalent = (nodes0, nodes1) => {
    ...
    return isEqual4(nodesWithoutIgnore0, nodesWithoutIgnore1);
  };
  const isInlineList = (nodes) => nodes.every((node) => isText4(node) || isInline(node));
  while (i < diff.length) {
    const chunk = diff[i];
    const op = chunk[0];
    const val = chunk[1];
...
import isEqual5 from "lodash/isEqual.js";
